...
- **바람** — 왼쪽 100%부터 무풍, 오른쪽 100%까지 조절합니다. 흐르는 빗물·미세 튀김과 구름 이동에 반영됩니다. 기본은 오른쪽 20%입니다.
...
다른 서버를 검사하려면 `QA_BASE_URL` 환경 변수를 지정합니다. 현재 단위 테스트 29개와 브라우저 테스트 16개를 통과했습니다. 검사 범위와 실행 환경은 [검증 기록](docs/verification.md)에 정리했습니다.
...
- 구름·차량 불빛은 합성 애니메이션이며 실제 교통·날씨 정보가 아닙니다. 사무실 소등은 사진의 밝은 픽셀과 도로·하늘의 근사 구분을 사용하므로 건물별 실제 조명 상태와 일치하지 않습니다. 기기 시간·시간대가 잘못 설정되면 조명 일정도 그 설정을 따릅니다.
