...
구름이 천천히 지나가고, 사진 속 도로 경로를 따라 차량 불빛이 움직이며, 일부 창문이 서서히 켜지고 꺼집니다. 비를 0으로 줄여도 도시는 움직입니다. 일시정지는 구름·차량·조명 변화까지 함께 멈춥니다.
...
도시와 유리 사이에는 세 깊이의 비가 내립니다. 거리별로 길이·속도·두께·밝기를 달리하고 옅은 빗안개를 더했습니다. 창밖 비는 도시보다 선명하고 유리 방울보다 부드럽게 표현하며, 물방울 내부의 굴절 장면에도 동일하게 반영됩니다. 사진 로딩 실패로 기본 야경이 표시될 때도 동작합니다.
...
다른 서버를 검사하려면 `QA_BASE_URL` 환경 변수를 지정합니다. 현재 단위 테스트 33개와 브라우저 테스트 18개를 통과했습니다. 검사 범위와 실행 환경은 [검증 기록](docs/verification.md)에 정리했습니다.
