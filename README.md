...
다른 서버를 검사하려면 `QA_BASE_URL` 환경 변수를 지정합니다. 현재 단위 테스트 41개와 브라우저 검사 22개를 통과했습니다(차량 그리기 방식 변경에 맞춰 기존 추적 검사를 수정 후 재검증). 검사 범위와 실행 환경은 [검증 기록](docs/verification.md)에 정리했습니다.
