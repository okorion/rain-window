...
도시 합성과 굴절용 배경 갱신은 모바일 최대 15fps, 데스크톱 최대 20fps로 별도 제한합니다. 사진의 점등·소등 버전과 구름 질감은 캐시하며 차량은 22대, 개별 창문 변화는 최대 1,800개 영역으로 제한합니다.
...
다른 서버를 검사하려면 `QA_BASE_URL` 환경 변수를 지정합니다. 현재 단위 테스트 44개와 브라우저 검사 26개를 통과했습니다. 검사 범위와 실행 환경은 [검증 기록](docs/verification.md)에 정리했습니다.
